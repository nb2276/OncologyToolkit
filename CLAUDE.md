...
- `tests.js` — test suite. Run with `node tests.js` (no test framework, no build — vanilla DOM-shimmed `node:vm` sandbox). Currently 612 assertions, 0 failures — and it must pass under any `TZ` (try `TZ=UTC node tests.js`); numeric expectations are computed independently, not copied from the code's own output.
...
- **PSA trailing below-detection result** — a rise followed by `<0.014` is a patient whose PSA has since gone undetectable. The fit can't see that row, so the recent-trend row is withheld (`trailingCensoredCount`) and the censored note says the doubling time describes the rise up to the last fitted date.
