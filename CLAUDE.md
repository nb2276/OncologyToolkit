...
- `psa.js` — weighted least-squares exponential fit (weights `w = y²`), 95% CI bands, click-to-query. Saves/restores full state to `history_psa` + the shareable URL: the whole `psaInput` textarea + `projectionYears` (`PSA_INPUT_IDS`), plus computed `psa-dt` / `psa-n` / `psa-span` extras for the label. `calculate(keepProjection)` skips the auto-projection-default when restoring; `restorePsaEntry` / `initPsaFromUrl` reconstruct from a history click or `?psaInput=…` (custom URL parse since `parseUrlParams` drops the non-numeric textarea text).
