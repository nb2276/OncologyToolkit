...
- `tests.js` — test suite. Run with `node tests.js` (no test framework, no build — vanilla DOM-shimmed `node:vm` sandbox). Currently 700 assertions, 0 failures — and it must pass under any `TZ` (try `TZ=UTC node tests.js`); numeric expectations are computed independently, not copied from the code's own output.
