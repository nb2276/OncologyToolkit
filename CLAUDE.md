...
- `tests.js` — test suite. Run with `node tests.js` (no test framework, no build — vanilla DOM-shimmed `node:vm` sandbox). Currently 523 assertions, 0 failures.
...
- **PSA input hardening** (from a production QA sweep): `parseLine` strips clock times before tokenising (`2024-01-15 08:30 4.5` used to split on the colon and read **08** as the PSA value), treats a whole field matching `GROUPED_NUMBER` as a thousands-separated number (`1,234` read as **1**) while still splitting a `date,value` CSV pair, strips quotes so an Excel paste (`"2024-01-15","4.5"`) works, and accepts month-name dates in both orders — the parsed table prints `Jan 1, 2024`, so without that the app could not read its own output back. `countUnparsedLines` drives a note for lines that could not be read at all, since they were vanishing silently between the textarea and the table.
- **PSA chart readouts**: `hoverTarget` resolves what the cursor is over — nearest plotted measurement (wins inside 14 px, because a point is a fact and a curve is a model), else the fitted or recent-trend curve within 30 px — and every readout is labelled with which one it came from. Before this the tooltip always computed from the overall fit while two curves were drawn, so pointing at the recent-trend line reported the fitted line's number.
